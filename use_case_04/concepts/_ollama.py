...
this exact Ollama HTTP API). Stdlib only (urllib), so a concept file installs
nothing. This is UC4's copy of UC1's _model.py; it imports nothing from ../demo/.

Needs, in one command:  ollama pull llama3.1  (daemon up; `ollama serve` if not).
On a 16 GB laptop use MODEL=llama3.2:1b to fit (llama3.1 has no 1b build; the small tag is
# Llama 3.2) - behaviour is the same, prose weaker.
"""
...
if not OLLAMA.startswith("http"):
    OLLAMA = "http://" + OLLAMA
if OLLAMA.startswith("http://0.0.0.0"):
...
def ask(system: str, user: str) -> str:
    ...
    matching src/redvault/config.py's ollama_temperature=0.0 for deterministic demos)."""
    body = {
